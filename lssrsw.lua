...
-- Dual-mode: works standalone (full GUI) or when loaded by the Unified Hub
...
local customWatchButton, playersTabBtn, logTabBtn
local playerList, logFrame, playerLayout, logLayout
local minimizeButton, killButton, resizeHandle
local editorOverlay, editorTitle, emojiInput, roleInput
local saveButton, cancelButton, removeButton

-- Forward declare cleanup so buttons can use it
local cleanup
...
if not IS_HUB then
	...
	minimizeButton = makeTopButton("-", Color3.fromRGB(70, 70, 80), -62)
	killButton = makeTopButton("X", Color3.fromRGB(130, 60, 65), -32)
	...
	customWatchButton = Instance.new("TextButton")
	...
	playersTabBtn = makeTab("Players", UDim2.new(), Color3.fromRGB(55, 120, 80))
	logTabBtn = makeTab("Log", UDim2.new(0.5, 0, 0, 0), Color3.fromRGB(55, 55, 65))
	...
	playerList, playerLayout = makeScroll()
	logFrame, logLayout = makeScroll()
	...
	-- Editor
	editorOverlay = Instance.new("Frame")
	...
	editorTitle = Instance.new("TextLabel")
	...
	emojiInput = makeInput("Emoji (example: ⚠️)", 45)
	roleInput = makeInput("Role/name (example: Suspicious)", 86)
		...
		if player.Parent and data then
			...
			if data.category == "Custom" then
				...
				editRowButton.MouseButton1Click:Connect(function()
					if editorOverlay and not editorOverlay.Visible then
						showEditor(player)
					end
				end)
			end
