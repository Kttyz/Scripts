-- Freecam (Dual Mode)
-- Works standalone (with GUI) or when loaded by the Unified Hub
...
local savedRootPart, savedRootAnchored
local savedHumanoid, savedWalkSpeed, savedJumpPower, savedJumpHeight, savedUseJumpPower, savedAutoRotate
...
_G.FreecamAPI = {
	...
	SetNormalSpeed = function(v)
		NORMAL_SPEED = v
	end,
	SetFastSpeed = function(v)
		FAST_SPEED = v
	end,
	GetNormalSpeed = function()
		return NORMAL_SPEED
	end,
	GetFastSpeed = function()
		return FAST_SPEED
	end,
}
...
connect(RunService.RenderStepped, function(dt)
	...
	if movement.Magnitude > 0 then
		local fast = UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or UserInputService:IsKeyDown(Enum.KeyCode.RightShift)
		cameraPosition += movement.Unit * (fast and FAST_SPEED or NORMAL_SPEED) * dt
	end
	camera.CFrame = CFrame.new(cameraPosition) * rotation
end)

-- =========================================================
--  STANDALONE GUI (only created when NOT loaded by hub)
-- =========================================================
if not IS_HUB then
	...
	connect(toggleBtn.MouseButton1Click, function()
		local state = _G.FreecamAPI.Toggle()
		statusLbl.Text = state and "Status: ON" or "Status: OFF"
		toggleBtn.BackgroundColor3 = state and Color3.fromRGB(55, 120, 80) or Color3.fromRGB(60, 100, 140)
	end)
	...
	connect(UserInputService.InputBegan, function(input)
		if waitingForKey and input.KeyCode ~= Enum.KeyCode.Unknown then
			...
			_G.FreecamAPI.SetKey(input.KeyCode)
	...
	connect(speedBox.FocusLost, function()
		...
		if n and n > 0 then
			NORMAL_SPEED = n
			_G.FreecamAPI.SetNormalSpeed(n)
		end
		speedBox.Text = tostring(NORMAL_SPEED)
	end)
	...
	-- Keep status in sync when using the keybind
	connect(RunService.Heartbeat, function()
		if enabled then
			statusLbl.Text = "Status: ON"
			toggleBtn.BackgroundColor3 = Color3.fromRGB(55, 120, 80)
		else
