...
local oldCleanup = _G.InvisiblePlayerToolCleanup
...
local function applyTransparency(player)
	...
	for _, part in ipairs(getCharacterParts(char)) do
		if not originalParts[player][part] then
			originalParts[player][part] = {Transparency = part.Transparency, LocalTransparencyModifier = part.LocalTransparencyModifier}
		end
...
local function manualCheckVisibility()
	...
	updateInvisibleList()
	if count == 0 then notify("Check", "No players became visible.") end
end
